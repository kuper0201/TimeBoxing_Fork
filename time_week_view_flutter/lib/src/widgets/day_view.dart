...
class _DayViewState extends ZoomableHeadersWidgetState<DayView> {
      ...
      mainWidget = DragTarget<FlutterWeekViewEvent>(
        ...
        onMove:(details) {
          ...
          if (gridGranularity > Duration.zero) {
            newStartTime = roundTimeToFitGrid(newStartTime, gridGranularity: gridGranularity);
          }
          
          setState(() {
            moved = newStartTime;
          });
          widget.dragAndDropOptions!.onEventMove(details.data, newStartTime);
        },
  ...
  void onEventResizeUpdate(FlutterWeekViewEvent event, double resizeOffset) {
    ...
    widget.resizeEventOptions!.onEventResizeMove(event, event.end);
  }
